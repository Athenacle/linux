...
enum { CARD_UNCHANGED = -1,
       CARD_OFF = 0,
       CARD_ON = 1,
};

static int load_state = CARD_UNCHANGED;
MODULE_PARM_DESC(load_state,
		 "Initial card state (0 = off, 1 = on, -1 = unchanged)");
module_param(load_state, int, 0400);
static int unload_state = CARD_UNCHANGED;
MODULE_PARM_DESC(unload_state,
		 "Card state on unload (0 = off, 1 = on, -1 = unchanged)");
module_param(unload_state, int, 0600);
static bool skip_optimus_dsm = false;
MODULE_PARM_DESC(skip_optimus_dsm,
		 "Skip probe of Optimus discrete DSM (default = false)");
...
static char *buffer_to_string(const char *buffer, size_t n, char *target)
{
	int i;
	for (i = 0; i < n; i++) {
		snprintf(target + i * 5, 5 * (n - i), "0x%02X,",
			 buffer ? buffer[i] & 0xFF : 0);
	}
	return target;
}
			 ...
			 int func, char args[4], uint32_t *result)
{
	...
	if (args) {
		...
		// Some implementations (Asus U36SD) seem to check the args
		// before the function ID and crash if it is not a buffer.
	...
	if (err) {
		...
		pr_warn("failed to evaluate %s._DSM {%s} 0x%X 0x%X {%s}: %s\n",
			...
			buffer_to_string(args, 4, args_str),
...
static int bbswitch_optimus_dsm(void)
{
	if (dsm_type == DSM_TYPE_OPTIMUS) {
		char args[] = {1, 0, 0, 3};
		u32 result = 0;
		if (acpi_call_dsm(dis_handle, acpi_optimus_dsm_muid, 0x100,
				  0x1A, args, &result)) {
			pr_warn("acpi_call_dsm failed");
			return 1;
		}
	}
	return 0;
...
static int bbswitch_acpi_off(void)
{
	if (dsm_type == DSM_TYPE_NVIDIA) {
		...
		if (acpi_call_dsm(dis_handle, acpi_nvidia_dsm_muid, 0x102, 0x3,
				  args, &result)) {
			// failure
			pr_warn("acpi_call_dsm failed");
			return 1;
...
static int is_card_disabled(void)
{
	...
	// bits are enabled, the device is assumed to be off
...
static void bbswitch_off(void)
{
	...
	// to prevent the system from possibly locking up, don't disable the
	// device if it's still in use by a driver (i.e. nouveau or nvidia)
	if (dis_dev->driver) {
	...
	if (bbswitch_optimus_dsm()) {
		pr_warn(
		    "Optimus ACPI call failed, the device is not disabled\n");
		return;
	}
	...
	if (bbswitch_acpi_off())
		pr_warn(
		    "The discrete card could not be disabled by a _DSM call\n");
}
...
static void bbswitch_on(void)
{
	...
	if (bbswitch_acpi_on())
		pr_warn(
		    "The discrete card could not be enabled by a _DSM call\n");
			       ...
			       unsigned long event_type, void *p)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		// enable the device before suspend to avoid the PCI config
		// space from being saved incorrectly
	...
	case PM_POST_RESTORE:
		// after suspend, the card is on, but if it was off before
		// suspend, disable it again
		if (dis_before_suspend_disabled) {
			dis_dev_get();
			bbswitch_off();
			dis_dev_put();
	...
	case PM_RESTORE_PREPARE:
		// deliberately don't do anything as it does not occur before
		// suspend nor hibernate, but before restoring a saved image. In
		// that case, either PM_POST_HIBERNATION or PM_POST_RESTORE will
		// be called
		break;
	}
	return 0;
...
static struct file_operations bbswitch_fops = {.open = bbswitch_proc_open,
					       .read = seq_read,
					       .write = bbswitch_proc_write,
					       .llseek = seq_lseek,
					       .release = single_release};

static struct notifier_block nb = {.notifier_call = &bbswitch_pm_handler};
...
static int __init bbswitch_init(void)
{
	...
	if (!skip_optimus_dsm &&
		...
		/* At least two Acer machines are known to use the intel ACPI
		 * handle with the legacy nvidia DSM */
		dis_handle = igd_handle;
		if (dis_handle &&
		    has_dsm_func(acpi_nvidia_dsm_muid, 0x102, 0x3)) {
			dsm_type = DSM_TYPE_NVIDIA;
	...
	acpi_entry =
	    proc_create("bbswitch", 0664, acpi_root_dir, &bbswitch_fops);
	if (acpi_entry == NULL) {
	...
	if (!is_card_disabled()) {
		/* We think the card is enabled, so ensure the kernel does as
		 * well */
		if (pci_enable_device(dis_dev))
			pr_warn("failed to enable %s\n",
				dev_name(&dis_dev->dev));
